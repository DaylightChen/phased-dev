...
2. **Confirm scope state exists.** If `docs/.phased-dev/state.json` is missing, tell the user to run `/init-project` first and stop.
...
3. **Verify the target scope exists.** Read `state.json` → `scopes`. If the requested ID is not in the list, stop and tell the user. Suggest:
   ...
   - For feature: `/start-feature <name>` to create it.
