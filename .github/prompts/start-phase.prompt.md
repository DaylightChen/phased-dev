...
1. **Confirm scope state exists.** If `docs/.phased-dev/state.json` is missing, tell the user to run `/init-project` first and stop.
...
4. **If `agent` is null** (this is the `implement` phase), stop and tell the user:
   > "The active scope `<id>` is in the `implement` phase. Use `/start-task <NN>` to run the dev loop on a specific task, not `/start-phase`."

5. **Check `phaseStatus`:**
   - `complete_awaiting_approval` — tell the user this phase already produced output and is awaiting their approval. Suggest `/advance-phase` or, if they want to redo the work, ask them to confirm before re-dispatching.
...
2. **Validate the agent.** Confirm the agent name from the pipeline entry corresponds to an actual agent file at `.github/agents/<agent-name>.md`. If the file doesn't exist, stop and tell the user the pipeline references a non-existent agent — the scope JSON may be corrupted or a custom template has a typo.

3. **Launch the agent.** Delegate to the custom agent named in the pipeline entry by invoking it through the `agent` tool (Copilot runs it as a sub-agent in its own context). Pass it:
   - The active scope ID
...
4. **When the agent returns:**
   ...
   - **If it reported completion:**
     ...
     4. Tell the user what was produced (paths from `paths` field) and remind them to run `/advance-phase` after review.
...
- For the `implement` phase, the per-task loop is handled by `/start-task`. This command refuses to dispatch in that case (step 4).
