---
name: ux-preview
description: Use to generate a static HTML preview from an approved UX markdown spec. Produces self-contained vanilla HTML/CSS files under the UX directory's preview/ subfolder for human visual review. Not consumed by downstream agents. Invoke after the ux phase spec is written.
...
- Report back with: which files were created (if any), which failed, and a note that the user can re-run the `generate-ux-preview` command to retry.
