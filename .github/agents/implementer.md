---
name: implementer
description: Use to write production code for a single task brief during the `implement` phase dev loop. Reads goal, context files, steps, and downstream dependencies from the brief, makes minimal targeted changes, and reports what was built. Does NOT write or run tests.
