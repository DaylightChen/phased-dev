---
name: tester
description: Use to write tests for the implementer's output, run the full test suite, and run type-checking during the `implement` phase dev loop, reporting pass/fail with specifics. Reads goal, acceptance criteria, downstream dependencies, and output files from the brief. Invoke after the implementer.
