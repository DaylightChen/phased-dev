...
description: Use for the `engineering` phase of a feature scope. Produces docs/features/<name>/engineering.md. Standard pipeline combines product + engineering in one spec; design-heavy pipeline focuses on engineering with the UX spec upstream. Reads the project AGENTS.md, engineering specs, and relevant code to fit the feature into the existing architecture.
...
3. **`AGENTS.md`** at project root — the project's workflow and conventions
...
8. **Relevant code** — based on what the feature touches, read the modules / files most likely to be affected. Use the search tool to find them.
