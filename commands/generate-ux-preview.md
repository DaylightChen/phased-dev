...
1. **Read scope state.** Read `docs/.phased-dev/state.json`. If missing, tell the user to run `/phased-dev:init-project` first and stop.
...
4. **Verify the markdown spec exists.** Glob `<paths.uxDir>*.md`. If no markdown files exist, stop — the UX spec hasn't been written yet. Tell the user to run `/phased-dev:start-phase` for the `ux` phase first (or, if the scope is past the `ux` phase, the spec may have been deleted).
...
Dispatch the `ux-preview` agent with:
- The active scope ID
